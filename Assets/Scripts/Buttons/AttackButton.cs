...
public class AttackButton : MonoBehaviour
{
    ...
    public void Attack()
    {
        ...
        gameObject.SetActive(false);
        // if(GameSystem.instance.getGameState() != GameSystem.GameState.gameOver)
        // {
        //     PlayerScript.instance.AttackAnim();
        //     AttackEffect.instance.startAttackAnim(_Color);
    ...
    {
        GetComponent<Button>().image.raycastTarget = false;

        for(int i = 0; i < colorButtons.Length; i++)
        {
            colorButtons[i].image.raycastTarget = false;
        }
    }

    public void enablingButtons()
    {
        GetComponent<Button>().image.raycastTarget = true;
        
        for(int i = 0; i < colorButtons.Length; i++)
        {
            colorButtons[i].image.raycastTarget = true;
        }
    }
